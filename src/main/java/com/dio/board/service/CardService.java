...
import com.dio.board.persistence.dao.BlockDAO;
...
import com.dio.board.persistence.exceptions.CardBlockedException;
import com.dio.board.persistence.exceptions.CardFinishedException;
import com.dio.board.persistence.exceptions.EntityNotFoundException;
...
import static com.dio.board.persistence.entity.BoardUnlockKindEnum.FINAL;
...
public class CardService {
    ...
    public void cardToNextColumn(final Long cardId, final List<BoardColumnInfoDTO> boardColumnInfo)throws SQLException {
        try {
            var dao = new CardDAO(connection);
            ...
            var currentColumn = boardColumnInfo.stream().filter(
                            bc -> bc.id().equals(dto.columnId()))
                    .findFirst().orElseThrow(
                            () -> new IllegalArgumentException("O card informado pertence a outro board"));
            ...
            var nextColumn = boardColumnInfo.stream().filter(bc -> bc.order() == currentColumn.order() + 1)
                    .findFirst().orElseThrow();
            dao.moveToColumn(cardId, currentColumn.id());
            connection.commit();
        } catch (SQLException ex) {
            connection.rollback();
            throw ex;
        ...
        public void block(final Long id,final String reason,final List<BoardColumnInfoDTO> boardColumnInfoDTO) throws SQLException{
            try{
                var dao = new CardDAO(connection);
                ...
                if(dto.blocked()){
                    var message = "o card %s ja foi bloqueado".formatted(id);
                    throw new CardBlockedException(message);
                }
                var currentColumn = boardColumnInfoDTO.stream().filter(
                        bc ->bc.id().equals(dto.columnId()))
                        .findFirst()
                        .orElseThrow();
